...
class ProfessionalProfileController extends ChangeNotifier
  ...
  int get totalOnlineSeconds {
    ...
    final nowSec =
        now.second + (now.minute * 60); // Unique second ID for the day
    ...
    if (_isOnline && _lastOnlineAt != null) {
      ...
      if (now.isBefore(_lastOnlineAt!)) {
        debugPrint(
          '⚠️ System Clock Jump Backward Detected: Resetting lastOnlineAt to NOW.',
        );
        _lastOnlineAt = now;
      } else if (_lastKnownTime != null &&
          now.difference(_lastKnownTime!).inHours > 2) {
        debugPrint(
          '⚠️ Suspicious Forward Clock Jump Detected (>2hrs): Resetting session start to NOW.',
        );
  ...
  int get lifetimeOnlineSeconds {
    if (_isOnline && _lastOnlineAt != null) {
      return _lifetimeSeconds +
          DateTime.now().difference(_lastOnlineAt!).inSeconds;
    }
    return _lifetimeSeconds;
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      debugPrint(
        '📡 App Resumed: Refreshing professional profile and timer UI.',
      );
  ...
  void checkDailyReset() {
    ...
    if (lastActivityDate != null && lastActivityDate != todayString) {
      debugPrint(
        '🌑 Midnight Reset: Clearing local rejection stats for new day ($todayString)',
      );
  ...
  Future<void> fetchProfile() async {
    ...
    try {
      ...
      if (newProfile != null) {
        ...
        if (_isOnline) {
          debugPrint(
            '🌐 ProfessionalProfileController: Syncing profile while ONLINE. (Backend says: ${newProfile.isOnline})',
          );
        } else {
          _isOnline = newProfile.isOnline;
          debugPrint(
            '🌐 ProfessionalProfileController: Syncing profile while OFFLINE. Updated to: $_isOnline',
          );
        }
  ...
  Future<void> _loadTimerState() async {
    try {
      ...
      if (unsyncedStr != null && unsyncedStr.isNotEmpty) {
        ...
        _unsyncedSessions = decoded
            .map((e) => pro_models.OnlineSession.fromJson(e))
            .toList();
      }
      ...
      if (_isOnline && _lastOnlineAt != null) {
        ...
        if (_lastKnownTime != null &&
            now.difference(_lastKnownTime!).inHours > 2) {
          debugPrint(
            '⚠️ Suspicious jump during resume. Ignoring offline time gained.',
          );
      ...
      notifyListeners();
      debugPrint(
        '⏱️ TimerState Loaded: elapsed=$_elapsedSeconds, online=$_isOnline, unsynced=${_unsyncedSessions.length}',
      );
    } catch (e) {
  ...
  Future<void> _saveTimerState() async {
    try {
      ...
      if (_lastKnownTime != null) {
        await prefs.setInt(
          'pro_last_known_time',
          _lastKnownTime!.millisecondsSinceEpoch,
        );
      } else {
      ...
      final encodedSessions = jsonEncode(
        _unsyncedSessions.map((e) => e.toJson()).toList(),
      );
      ...
      if (_lastOnlineAt != null) {
        await prefs.setInt(
          'pro_last_online_at',
          _lastOnlineAt!.millisecondsSinceEpoch,
        );
      } else {
  ...
  void _goOffline() {
    if (_lastOnlineAt != null) {
      ...
      if (diff > 0) {
        // Don't save 0 second sessions
        _unsyncedSessions.add(
          pro_models.OnlineSession(startTime: _lastOnlineAt!, endTime: now),
        );
      }
    }
    _isOnline = false;
  ...
  Future<bool> toggleAvailability(bool online) async {
    debugPrint(
      '🔘 ProfessionalProfileController: Toggling Availability to: $online',
    );

    if (_profile != null && _profile!.status.toLowerCase() != 'active') {
      _isOnline = false;
      _error = "Your account is suspended";
      debugPrint('🚫 Blocked: Cannot toggle online because account is ${_profile!.status}.');
      notifyListeners();
      return false;
    }
    ...
    try {
      ...
      if (res['success'] == true) {
        // UI updates handled strictly via `updateRealtimeStatus` event
        return true;
      } else {
  ...
  void updateRealtimeStatus(Map data) {
    if (_profile == null) return;
    
    debugPrint('🔔 updateRealtimeStatus received: $data');

    final newStatusRaw = data['status'];
    final newIsOnlineRaw = data['is_online'] ?? data['availability_status'];
    final bool newIsOnline = newIsOnlineRaw == true || newIsOnlineRaw == 1 || newIsOnlineRaw == '1' || newIsOnlineRaw == 'true';

    bool isSuspended = (newStatusRaw != null && newStatusRaw.toString().toLowerCase() == 'suspended');

    _profile = _profile!.copyWith(
      status: newStatusRaw?.toString() ?? _profile!.status,
      isSuspended: isSuspended,
    );

    if (isSuspended) {
      if (_isOnline) _goOffline();
      _error = "Account suspended by Admin";
    } else {
      if (newIsOnline && !_isOnline) {
        _goOnline();
        _startHeartbeat();
      } else if (!newIsOnline && _isOnline) {
        _goOffline();
        _heartbeatTimer?.cancel();
      }
    }
  ...
  Future<void> _syncWithBackend(int currentSeconds) async {
    ...
    debugPrint(
      '📡 Syncing: Reporting $currentSeconds online seconds to backend.',
    );
    try {
      ...
      if (_unsyncedSessions.isNotEmpty) {
        ...
        debugPrint(
          '📡 Synced ${_unsyncedSessions.length} pending offline sessions.',
        );
  ...
  Future<bool> updateBankDetails(
    ...
    () =>
        ProfessionalApiService.updateBankDetails(data, proofImage: proofImage),
  );
  Future<bool> updateUPIDetails(
    Map<String, String> data, {
    XFile? screenshot,
  }) async => _performUpdate(
    () => ProfessionalApiService.updateUPIDetails(data, screenshot: screenshot),
  );
